...
interface IGame{
    ...
    function mintPokemon(address to, address addressPokemon) external;
...
contract Game{
    ...
    function mintPokemon(address to, address addressPokemon) external {
        require(balancePokeballs[msg.sender] >= 1, "You don't have enough pokeballs");
        owners[addressPokemon] = to;
        token.mint(Player(to).clientAddress(), IPokemon(addressPokemon).pokemonId(), 1);
    ...
    function mintBatchPokemon(address to, address[] memory addressesPokemon, uint256[] memory amounts) external {
        ...
        require(balancePokeballs[msg.sender] >= sumAmounts, "You don't have enough pokeballs");
    ...
    function transferBatchPokemon(
        ...
        require(addressesPokemon.length == amounts.length, "The length of the addressesPokemon array is not equal to the length of the amounts array");
        uint256[] memory _tokenIds = new uint256[](amounts.length);
        for(uint256 i = 0; i < addressesPokemon.length; i++){
            uint256 _tokenId = IPokemon(addressesPokemon[i]).pokemonId();
            _tokenIds[i] = _tokenId;
