...
class Player_Map:
    def __init__(self, x: int, y: int, width: int, height: int, name: str, address: str, pokemons: list):
        self.x = x
        self.y = y
        self.name = name
        ...
        self.width = width
        self.height = height
        self.text_color = text_id[4]
        self.border_color = (1, 101, 1)
        self.height_rect = self.height * 1 / 3 / 4 - 1

    def draw(self, screen: "SURFACE"):
        """
        Метод отрисовки карточки игрока
        ...
        render_line = font.render(f"Pokemons count: {len(self.pokemons)}", True, self.text_color)
